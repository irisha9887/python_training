...
def test_delete_some_group(app, db, check_ui):
    with pytest.allure.step('Given Home page is opened'):
    ...
    with pytest.allure.step('When I delete the group %s from the list' % group.id):
        app.group.delete_group_by_id(group.id)
    with pytest.allure.step('Then the new group list is equal to the old list group without the deleted group'):
